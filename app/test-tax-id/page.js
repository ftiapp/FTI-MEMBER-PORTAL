    ...
    <div className="container mx-auto px-4 py-8 max-w-2xl">
        ...
        {result && (
          <div className={`mt-6 p-4 rounded-md ${result.valid ? "bg-green-50" : "bg-red-50"}`}>
            ...
            <div className="mt-2 text-sm">
              <p>
                <strong>สถานะ:</strong> {result.valid ? "ผ่านการตรวจสอบ" : "ไม่ผ่านการตรวจสอบ"}
              </p>
              {result.message && (
                <p>
                  <strong>ข้อความ:</strong> {result.message}
                </p>
              )}
              {result.status !== undefined && (
                <p>
                  <strong>สถานะในระบบ:</strong> {result.status}
                </p>
              )}
              {result.exists !== undefined && (
                <p>
                  <strong>มีในระบบ:</strong> {result.exists ? "ใช่" : "ไม่"}
                </p>
              )}
            </div>
          </div>
      ...
      <div className="mt-8 bg-blue-50 p-6 rounded-lg border border-blue-100">
        ...
        <ul className="list-disc pl-5 space-y-2 text-sm text-blue-800">
          ...
          <li>
            ถ้ามีในระบบแล้ว จะตรวจสอบสถานะว่าเป็น &quot;รอพิจารณา&quot; (0) หรือ &quot;อนุมัติ&quot;
            (1) หรือไม่
          </li>
          <li>
            ถ้าสถานะเป็น &quot;รอพิจารณา&quot; หรือ &quot;อนุมัติ&quot; จะไม่สามารถสมัครซ้ำได้
          </li>
