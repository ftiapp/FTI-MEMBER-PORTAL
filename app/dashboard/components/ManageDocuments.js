...
export default function ManageDocuments() {
  ...
  const handleDeleteDocument = async (documentId) => {
    ...
    try {
      const response = await fetch(`/api/member/delete-document?id=${documentId}&userId=${user.id}`, {
        method: 'DELETE',
      });
                ...
                <div key={doc.id} className="p-4 border rounded-lg flex justify-between items-center">
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">อัพโหลดเมื่อ {formatDate(doc.uploaded_at)}</p>
                    </div>
                  </div>
