...
const MemberDetail = ({ userId }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!companiesResponse.ok) {
        throw new Error(
          `ไม่สามารถดึงข้อมูลบริษัทที่ได้รับการอนุมัติได้ (${companiesResponse.status})`,
        );
      }
  ...
  // Loading state
  if (loading) {
    return <LoadingOverlay isVisible={true} message="กำลังโหลดข้อมูลสมาชิก..." inline={true} />;
  }
      ...
      <AnimatePresence mode="wait" initial={false}>
        {tableView ? (
          ...
          >
            ...
            <ApprovedCompaniesTable companies={currentItems} formatDate={formatDate} />
