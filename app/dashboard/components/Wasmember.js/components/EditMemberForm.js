...
const EditMemberForm = ({ submission, onClose, onSuccess }) => {
      ...
      try {
        ...
        if (data.success) {
          ...
          if (memberData.MEMBER_CODE) {
            const docsResponse = await fetch(`/api/member/get-documents?memberCode=${memberData.MEMBER_CODE}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success('แก้ไขข้อมูลเรียบร้อยแล้ว เจ้าหน้าที่จะตรวจสอบและติดต่อกลับภายใน 1-2 วันทำการ');
        ...
        {isLoading ? (
          ...
          <div>
            <label htmlFor="documentFile" className="block text-sm font-medium text-gray-700 mb-1">
              เอกสารยืนยันตัวตน {existingDocuments.length === 0 && <span className="text-red-500">*</span>}
            </label>
            <div className={`mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-dashed rounded-md ${formErrors.documentFile ? 'border-red-500' : 'border-gray-300'}`}>
