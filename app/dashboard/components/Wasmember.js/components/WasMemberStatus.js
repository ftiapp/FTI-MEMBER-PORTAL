...
import { FaCheckCircle, FaTimesCircle, FaHourglassHalf, FaInfoCircle, FaEdit, FaFileAlt } from 'react-icons/fa';
  ...
  const fetchDocumentsForVerifications = async (verificationsList) => {
    ...
    for (const verification of verificationsList) {
      if (verification.MEMBER_CODE) {
        try {
          const response = await fetch(`/api/member/get-documents?memberCode=${verification.MEMBER_CODE}`);
          if (response.ok) {
            const data = await response.json();
      ...
      {verifications.length === 0 ? (
        ...
        <div className="space-y-4">
          {verifications.map((verification) => (
            <div key={verification.id} className={`border rounded-lg p-4 hover:shadow-md transition-shadow ${verification.Admin_Submit === 2 ? 'bg-red-50 border-red-200' : verification.Admin_Submit === 1 ? 'bg-green-50 border-green-200' : 'bg-blue-50 border-blue-200'}`}>
