...
export default function MemberSearchField({
  ...
  const getCompanyName = (item, needle = "") => {
    const thaiCandidates = [item.COMP_NAME_TH, item.COMPANY_NAME_TH, item.COMPANY_NAME].filter(
      Boolean,
    );
  ...
  const performSearch = async (term) => {
    if (!term || term.length < 2) {
      // Match the original 2 character minimum
      setSearchResults([]);
                  ...
                  >
                    <div className="flex justify-between">
                      <div className="font-medium">
                        {highlightText(
                          getDisplayName(result, searchTerm) || "ไม่ระบุชื่อ",
                          searchTerm,
                        )}
                      </div>
                      {isNonSelectable && (
                        <span
                          className={`text-xs px-2 py-1 rounded-full ${status === "pending" ? "bg-yellow-100 text-yellow-800" : "bg-green-100 text-green-800"}`}
                        >
                    ...
                    <div className="text-xs text-gray-500 flex justify-between">
                      <span>
                        รหัสสมาชิก: {highlightText(result.MEMBER_CODE || "ไม่ระบุ", searchTerm)}
                      </span>
