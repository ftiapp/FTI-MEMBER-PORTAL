...
const ThaiCertificate = ({ memberData, formatThaiDate }) => {
  ...
  return (
    <div className="certificate-container relative w-full bg-white p-8 min-h-screen">
      ...
      <div
        className="certificate-content relative z-10 text-black"
        style={{ fontFamily: "Sarabun, sans-serif", fontSize: "16px", lineHeight: "1.6" }}
      >
        {/* เลขที่อ้างอิง */}
        <div className="certificate-header-ref text-left mb-8 mt-8">
          <p>
            ที่ {new Date().getDate()}/{new Date().getMonth() + 1}/{new Date().getFullYear() + 543}
          </p>
        </div>
        ...
        <div className="certificate-body px-16 mb-20">
          <div className="certificate-body mb-8">
            ...
            <p className="text-center mb-6">เป็นสมาชิกสภาอุตสาหกรรมแห่งประเทศไทย</p>

            <p className="text-center mb-6">
              ประเภท {getFullMemberType(memberData?.company_type) || "........"} เลขที่สมาชิก{" "}
              {memberData?.MEMBER_CODE || "..............."} ตั้งแต่วันที่{" "}
              {memberData?.JOIN_DATE
                ? formatThaiDate(memberData.JOIN_DATE)
                : "..........................."}
            </p>
            ...
            <p className="mb-10 text-right">ออกให้ ณ วันที่ {currentDate}</p>
          </div>
        </div>
