  ...
  useEffect(() => {
    ...
    if (verifications.length > 0) {
      const verificationIds = new Set(
        initialOperations.filter((op) => op.title?.includes("ยืนยันสมาชิกเดิม")).map((op) => op.id),
      );
    ...
    if (contactMessageStatus && contactMessageStatus.length > 0) {
      ...
      const existingContactIds = new Set(
        initialOperations.filter((op) => op.title === "ติดต่อเจ้าหน้าที่").map((op) => op.id),
      );
    ...
    if (addressUpdates && addressUpdates.length > 0) {
      ...
      const existingAddressUpdateIds = new Set(
        initialOperations.filter((op) => op.title === "แก้ไขข้อมูลสมาชิก").map((op) => op.id),
      );
      ...
      const addressUpdateOps = addressUpdates
        .filter(
          (update) =>
            update.status !== "none" &&
            update.status !== "error" &&
            !existingAddressUpdateIds.has(update.id),
        )
        .map((update) => ({
          id: update.id,
    ...
    if (productUpdates && productUpdates.length > 0) {
      ...
      const existingProductUpdateIds = new Set(
        initialOperations.filter((op) => op.title === "แก้ไขข้อมูลสินค้า").map((op) => op.id),
      );
      ...
      const productUpdateOps = productUpdates
        .filter(
          (update) =>
            update.status !== "none" &&
            update.status !== "error" &&
            !existingProductUpdateIds.has(update.id),
        )
        .map((update) => ({
          id: update.id,
    ...
    if (socialMediaUpdates && socialMediaUpdates.length > 0) {
      ...
      const existingSocialMediaIds = new Set(
        initialOperations.filter((op) => op.title === "อัปเดตโซเชียลมีเดีย").map((op) => op.id),
      );
      ...
      const socialMediaOps = socialMediaUpdates
        .filter(
          (update) =>
            update.status !== "none" &&
            update.status !== "error" &&
            !existingSocialMediaIds.has(update.id),
        )
        .map((update) => ({
          id: update.id,
          ...
          description:
            update.description ||
            `บริษัท: ${update.company_name || update.member_code} (${update.items?.length || 0} รายการ)`,
    ...
    if (logoUpdates && logoUpdates.length > 0) {
      ...
      const existingLogoIds = new Set(
        initialOperations.filter((op) => op.title === "อัปเดตโลโก้บริษัท").map((op) => op.id),
      );
    ...
    if (tsicUpdates && tsicUpdates.length > 0) {
      ...
      const existingTsicIds = new Set(
        initialOperations.filter((op) => op.title === "อัปเดตรหัส TSIC").map((op) => op.id),
      );

      // Filter valid updates
      const validTsicUpdates = tsicUpdates.filter(
        (update) =>
          update.status !== "none" && update.status !== "error" && !existingTsicIds.has(update.id),
      );
    ...
    const restOps = initialOperations.map((op) => ({
      ...op,
      type: op.title?.includes("ยืนยันสมาชิกเดิม")
        ? "ยืนยันสมาชิกเดิม"
        : op.title === "ติดต่อเจ้าหน้าที่"
          ? "ติดต่อเจ้าหน้าที่"
          : op.title === "แก้ไขข้อมูลสมาชิก"
            ? "แก้ไขข้อมูลสมาชิก"
            : op.title === "แก้ไขข้อมูลสินค้า"
              ? "แก้ไขข้อมูลสินค้า"
              : op.title === "อัปเดตโซเชียลมีเดีย"
                ? "อัปเดตโซเชียลมีเดีย"
                : op.title === "อัปเดตโลโก้บริษัท"
                  ? "อัปเดตโลโก้บริษัท"
                  : op.title === "อัปเดตรหัส TSIC"
                    ? "อัปเดตรหัส TSIC"
                    : "แก้ไขข้อมูลส่วนตัว",
    }));
