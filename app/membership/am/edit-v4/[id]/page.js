...
import AMMembershipForm from "@/app/membership/am/components/AMMembershipForm";
import { submitAMMembershipDocumentsUpdate } from "@/app/membership/am/components/AMFormSubmission";
...
export default function EditAMApplicationV4() {
    ...
    async (data) => {
      try {
        ...
        if (comment && comment.trim()) {
          const commentRes = await fetch(`/api/membership/am/${params.id}/comment`, {
            method: "POST",
            headers: {
        ...
        const res = await fetch(`/api/membership/am-v4/${params.id}/update`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        const docResult = await submitAMMembershipDocumentsUpdate(data, params.id);
  ...
  const fetchApplication = async () => {
    try {
      ...
      const res = await fetch(`/api/membership/am/summary/${params.id}`);
      const result = await res.json();
      ...
      console.error("[AM-V4] Load error:", e);
  ...
  const handleSendContactMessage = useCallback(
    async (message) => {
      ...
      try {
        ...
        const res = await fetch(`/api/membership/am/${params.id}/comment`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        console.error("[AM-V4] Contact admin error:", e);
    ...
    <>
      ...
      <main className="min-h-screen bg-gray-50">
        <div className="relative bg-gradient-to-r from-blue-900 to-blue-700 text-white py-16 md:py-24">
          ...
          <div className="container mx-auto px-4 relative z-10 max-w-5xl">
            <h1 className="text-3xl md:text-5xl font-bold mb-4 text-center">
              แก้ไข - ใบสมัครสมาชิกประเภท สามัญ-สมาคมการค้า
            </h1>
            <motion.div
        ...
        <div className="container mx-auto px-4 py-12 md:py-16">
          ...
          >
            <MembershipConversationHistory membershipType="am" membershipId={params.id} />
          </motion.div>
