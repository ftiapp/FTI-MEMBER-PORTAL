...
export default function CompanyAddressInfo({ formData, setFormData, errors, isAutofill }) {
  ...
  useEffect(() => {
    ...
    if (errors.addresses && typeof errors.addresses === "object") {
      ...
      const firstTypeWithError = typeKeys.find(
        (t) => errors.addresses[t] && Object.keys(errors.addresses[t] || {}).length > 0,
      );
  ...
  const handleContactInputChange = (e) => {
    ...
    setFormData((prev) => {
      ...
      if (
        ["companyPhone", "companyPhoneExtension", "companyEmail", "companyWebsite"].includes(name)
      ) {
    ...
    async (subDistrict) => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/postal-code?subDistrict=${encodeURIComponent(subDistrict.trim())}`,
        );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <SearchableDropdown
                ...
                autoFillNote={
                  isAutofill && currentAddress?.postalCode ? "* ข้อมูลถูกดึงอัตโนมัติ" : null
                }
                disabled={isLoading}
              />
            </div>
