  ...
  try {
    ...
    if (formData.addresses && typeof formData.addresses === "object") {
      try {
        ...
        const normalizedAddresses = Object.entries(formData.addresses).reduce(
          (acc, [type, addr]) => {
            const a = { ...(addr || {}) };
    ...
    if (formData.representative) {
      ...
      formDataToSubmit.append(
        "representativeFirstNameTh",
        formData.representative.firstNameThai || "",
      );
      formDataToSubmit.append(
        "representativeLastNameTh",
        formData.representative.lastNameThai || "",
      );
      formDataToSubmit.append(
      ...
      formDataToSubmit.append(
        ...
        formData.representative.phoneExtension || "",
      );
      ...
      formDataToSubmit.append(
        "representativePrenameOther",
        formData.representative.prenameOther || "",
      );
      // NEW: English other prename for representative
      formDataToSubmit.append(
        "representativePrenameOtherEn",
        formData.representative.prenameOtherEn || "",
      );
      ...
      formDataToSubmit.append(
        "representative_prename_other",
        formData.representative.prenameOther || "",
      );
      // NEW snake_case for English other prename
      formDataToSubmit.append(
        "representative_prename_other_en",
        formData.representative.prenameOtherEn || "",
      );
      ...
      formDataToSubmit.append(
        "representativePhoneExtension",
        formData.representativePhoneExtension || "",
      );
      ...
      formDataToSubmit.append(
        "representativePrenameOther",
        formData.representativePrenameOther || "",
      );
      // NEW: English other prename (flat)
      formDataToSubmit.append(
        "representativePrenameOtherEn",
        formData.representativePrenameOtherEn || "",
      );
      // ✅ Also send snake_case duplicates for backend compatibility
    ...
    if (formData.industrialGroupNames && Array.isArray(formData.industrialGroupNames)) {
      ...
      industryGroupNamesToSend = formData.industrialGroupNames.map((name) =>
    ...
    if (formData.provincialChapterNames && Array.isArray(formData.provincialChapterNames)) {
      // ✅ Keep array length aligned with IDs; do not filter out empties
      provinceChapterNamesToSend = formData.provincialChapterNames.map((name) =>
        typeof name === "string" ? name : "",
      );
    } else if (formData.provincialCouncilNames && Array.isArray(formData.provincialCouncilNames)) {
      // Backward compatibility
      provinceChapterNamesToSend = formData.provincialCouncilNames.map((name) =>
        typeof name === "string" ? name : "",
      );
    }
    ...
    if (typeof formData.authorizedSignatoryPositionTh === "string") {
      ...
      // snake_case for compatibility
      formDataToSubmit.append(
        "authorized_signatory_position_th",
        formData.authorizedSignatoryPositionTh,
      );
    }
    if (typeof formData.authorizedSignatoryPositionEn === "string") {
      ...
      // snake_case for compatibility
      formDataToSubmit.append(
        "authorized_signatory_position_en",
        formData.authorizedSignatoryPositionEn,
      );
    }
    ...
    if (
      formData.companyRegistrationDocument &&
      formData.companyRegistrationDocument instanceof File
    ) {
    ...
    try {
      const memberData = {
        ...
        applicantName:
          `${formData.firstNameThai || formData.firstNameTh || ""} ${formData.lastNameThai || formData.lastNameTh || ""}`.trim(),
        companyNameTh:
          `${formData.firstNameThai || formData.firstNameTh || ""} ${formData.lastNameThai || formData.lastNameTh || ""}`.trim(),
        companyNameEn:
