...
export default function SummarySection({ 
  ...
  const getIndustrialGroupNames = () => {
    ...
    if (formData.industrialGroupId && Array.isArray(formData.industrialGroupId) && formData.industrialGroupId.length > 0) {
  ...
  const getProvincialChapterNames = () => {
    ...
    if (formData.provincialChapterId && Array.isArray(formData.provincialChapterId) && formData.provincialChapterId.length > 0) {
      ...
      <Section title="ที่อยู่" className="mt-6">
        {(() => {
          ...
          const renderAddressBlock = (type) => {
            const addressData = (formData.addresses && formData.addresses[type]) ? formData.addresses[type] : {};
          ...
          if (formData.addresses && typeof formData.addresses === 'object') {
            return (
              <div className="space-y-6">
                {['2', '1', '3'].map((t) => renderAddressBlock(t))}
              </div>
            );
          }
