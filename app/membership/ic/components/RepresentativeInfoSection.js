...
export default function RepresentativeInfoSection({ formData, setFormData, errors }) {
  ...
  const getErr = (camelKey, snakeKey) =>
    representativeErrors?.[camelKey] ?? representativeErrors?.[snakeKey];
  ...
  const handleRepresentativeChange = (field, value) => {
    ...
    if (
      (field === "firstNameThai" || field === "lastNameThai" || field === "prenameOther") &&
      value
    ) {
    ...
    if (
      (field === "firstNameEng" || field === "lastNameEng" || field === "prenameOtherEn") &&
      value
    ) {
    ...
    if (field === "prenameTh") {
      ...
      if (
        !updatedRepresentative.prenameEn ||
        thaiToEnglishMap[value] !== updatedRepresentative.prenameEn
      ) {
      ...
      if (
        !updatedRepresentative.prenameTh ||
        englishToThaiMap[value] !== updatedRepresentative.prenameTh
      ) {
  ...
  useEffect(() => {
    if (representativeErrors) {
      if (getErr("prenameTh", "prename_th") && prenameThRef.current) {
        ...
        toast.error(getErr("prenameTh", "prename_th") || "กรุณาเลือกคำนำหน้าชื่อภาษาไทย", {
          position: "top-right",
          style: { zIndex: 100000 },
        });
      } else if (getErr("prenameEn", "prename_en") && prenameEnRef.current) {
        prenameEnRef.current.scrollIntoView({ behavior: "smooth", block: "center" });
        toast.error(getErr("prenameEn", "prename_en") || "กรุณาเลือกคำนำหน้าชื่อภาษาอังกฤษ", {
          position: "top-right",
          style: { zIndex: 100000 },
        });
        ...
        toast.error(getErr("prenameOther", "prename_other") || "กรุณาระบุคำนำหน้าชื่ออื่นๆ", {
          position: "top-right",
          style: { zIndex: 100000 },
        });
        ...
        toast.error(
          getErr("prenameOtherEn", "prename_other_en") || "Please specify prename in English",
          { position: "top-right", style: { zIndex: 100000 } },
        );
      }
    }
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    ...
                    <div>
                      <input
                        ...
                        onChange={(e) =>
                          handleRepresentativeChange("phoneExtension", e.target.value)
                        }
