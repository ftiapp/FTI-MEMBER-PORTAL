...
export default function EditOCApplicationV4() {
  ...
  const handleEditSubmit = useCallback(
    async (data) => {
      try {
        // 1) Send user comment to admin (required, enforced in form)
        const comment = data.userResubmissionComment;
        if (comment && comment.trim()) {
          const commentRes = await fetch(`/api/membership/oc/${params.id}/comment`, {
            method: "POST",
            headers: {
          ...
          if (!commentRes.ok || !commentResult.success) {
            return {
              success: false,
              message:
                commentResult.message || "ไม่สามารถส่งข้อความถึงผู้ดูแลระบบได้ กรุณาลองใหม่อีกครั้ง",
            };
          }
        } else {
          return {
        ...
        // 2) Update main & related tables via JSON endpoint
        const res = await fetch(`/api/membership/oc-v4/${params.id}/update`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        // 3) Update documents & Cloudinary (best-effort)
        const docResult = await submitOCMembershipDocumentsUpdate(data, params.id);
        ...
        console.error("[OC-V4] Edit submit error:", e);
  ...
  const fetchApplication = async () => {
    try {
      ...
      const res = await fetch(`/api/membership/oc/summary/${params.id}`);
      const result = await res.json();
      ...
      // Summary API คืนข้อมูล OC เต็มจาก main + tables ที่เกี่ยวข้องแล้ว
      setFormData(result.data || {});
    } catch (e) {
      console.error("[OC-V4] Load error:", e);
      ...
      <main className="min-h-screen bg-gray-50">
        ...
        <div className="container mx-auto px-4 py-12 md:py-16">
          ...
          >
            ...
            <div className="relative z-10">
              {/* ใช้ OCMembershipForm แบบ external state, และใช้ isRejectedMode เพื่อตัด Tax ID uniqueness check */}
              <OCMembershipForm
