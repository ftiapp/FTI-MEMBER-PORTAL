...
export function useProductUpdateRequests() {
  ...
  const fetchRequests = useCallback(async (skipCache = false) => {
    ...
    try {
      ...
      const cacheKey = getCacheKey(pagination.page, pagination.limit, filters.status, filters.search);
  ...
  }, [pagination.page, pagination.limit, filters.status, filters.search, cache, getCacheKey, isCacheValid, router]);
  ...
  const approveRequest = useCallback(async (id, adminNotes) => {
    try {
      setLoading(true);
  ...
  const handleReject = useCallback(async (id, rejectReason) => {
    try {
      setLoading(true);
