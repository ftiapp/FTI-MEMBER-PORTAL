          ...
          <div className="bg-purple-50 p-4 rounded-lg border border-purple-100">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">{stats.averageLoginCount ? Number(stats.averageLoginCount).toFixed(1) : 0} ครั้ง</p>
              </div>
              ...
              <div>
                ...
                <p className="text-xl font-bold text-indigo-600 truncate">
                  {stats.mostActiveUser ? 
                    `${stats.mostActiveUser.firstname || ''} ${stats.mostActiveUser.lastname || ''}`.trim() || 'ไม่มีข้อมูล' 
                    : 'ไม่มีข้อมูล'}
                </p>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {users.length > 0 ? (
                    users.map((user, index) => (
                      <tr key={user.id} className={`${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50`}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                            ...
                            {user.status === 'active' ? 'ใช้งาน' : user.status === 'pending' ? 'รอการยืนยัน' : 'ไม่ใช้งาน'}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {user.login_count || 0}
