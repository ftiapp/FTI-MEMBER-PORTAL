...
/**
 * ContactMessageStats Component
 * 
 * Displays a bar graph of contact message statistics by status (unread, read, replied)
 */
export default function ContactMessageStats({ title }) {
  ...
  useEffect(() => {
    const fetchContactMessageStats = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/admin/contact-message-stats', {
          cache: 'no-store',
          next: { revalidate: 0 }
        });
        ...
        if (data.success) {
          ...
          throw new Error(data.message || 'Failed to fetch contact message statistics');
        }
      } catch (err) {
        ...
        callbacks: {
          label: function(context) {
            ...
            const percentage = stats.totalCount > 0 
              ? Math.round((value / stats.totalCount) * 100) 
              : 0;
  ...
  // Chart data
  const chartData = {
    labels: Object.keys(stats).filter(key => key !== 'totalCount').map(key => statusLabels[key]),
    datasets: [
      {
        data: Object.keys(stats).filter(key => key !== 'totalCount').map(key => stats[key]),
        backgroundColor: Object.keys(stats).filter(key => key !== 'totalCount').map(key => statusColors[key]),
        borderColor: Object.keys(stats).filter(key => key !== 'totalCount').map(key => statusColors[key].replace('0.7', '1')),
        borderWidth: 1,
      },
    ],
  };
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-lg shadow-md p-6 mb-8">
        <h2 className="text-xl font-bold text-gray-800 mb-4">{title || 'สถิติข้อความติดต่อ'}</h2>
      ...
      {stats.totalCount === 0 ? (
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4 mt-6">
            {Object.keys(stats).filter(key => key !== 'totalCount').map((status) => (
              <div 
                key={status} 
