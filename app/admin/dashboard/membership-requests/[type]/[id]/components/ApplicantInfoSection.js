      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <p className="font-medium">{application.company_name_th || application.companyNameTh || application.associationNameTh || '-'}</p>
          </div>
          <div>
            <p className="text-gray-600 text-sm">ชื่อบริษัท (อังกฤษ)</p>
            <p className="font-medium">{application.company_name_en || application.companyNameEn || application.associationNameEn || '-'}</p>
          </div>
          <div>
          ...
          {type === 'oc' && (
            <>
              ...
              <div>
                ...
                <p className="font-medium">{application.number_of_employees || application.numberOfEmployees || '-'}</p>
              </div>
            </>
          )}
          <div>
            <p className="text-gray-600 text-sm">อีเมล</p>
            <p className="font-medium">{application.company_email || application.companyEmail || application.email || '-'}</p>
          </div>
          <div>
            <p className="text-gray-600 text-sm">เบอร์โทรศัพท์</p>
            <p className="font-medium">{application.company_phone || application.companyPhone || application.phone || '-'}</p>
          </div>
          <div>
            <p className="text-gray-600 text-sm">เว็บไซต์</p>
            <p className="font-medium">{application.company_website || application.companyWebsite || application.website || '-'}</p>
          </div>
        </div>
      </div>
    );
