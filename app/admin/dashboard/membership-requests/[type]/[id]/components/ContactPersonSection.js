        ...
        {application.contactPersons.map((contact, index) => (
          <div key={index} className="border-2 border-blue-200 rounded-lg p-6 bg-blue-50">
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-lg font-bold text-blue-900">ผู้ติดต่อคนที่ {index + 1}</h4>
              <span className="bg-blue-100 text-blue-800 text-sm font-medium px-3 py-1 rounded-full">
                {getContactTypeDisplay(
                  contact.type_contact_id,
                  contact.type_contact_name,
                  contact.type_contact_other_detail,
                )}
              </span>
            </div>
                ...
                <p className="text-sm text-gray-900">
                  ...
                  {(contact.phone_extension || contact.phoneExtension) && (
                    <span className="text-blue-600 ml-2">
                      ต่อ {contact.phone_extension || contact.phoneExtension}
                    </span>
                  )}
                </p>
              </div>
