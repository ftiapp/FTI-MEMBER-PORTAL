      ...
      <div className="space-y-4">
        {repsArray.map((rep, index) => {
          ...
          const isPrimary = (
            ...
            rep.rep_order === 1 || rep.repOrder === 1 ||
            rep.isPrimary === true || rep.order === 1
          );
          const isSecondary = (
            rep.is_primary === 0 || rep.is_primary === false ||
            rep.rep_order === 2 || rep.repOrder === 2 ||
            rep.order === 2
          );

          const repType = isPrimary
            ? 'ผู้แทน 1 (หลัก)'
            : (isSecondary ? 'ผู้แทน 2 (รอง)' : `ผู้แทน ${index + 1}`);
