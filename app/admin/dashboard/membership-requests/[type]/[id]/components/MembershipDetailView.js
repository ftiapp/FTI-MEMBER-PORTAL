...
const MembershipDetailView = ({
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-blue-100 text-lg">
              {(() => {
                const t = getMemberTypeInfo(type);
                return `ประเภท: ${t.code} (${t.name})`;
              })()}
            </p>
      ...
      <IndustrialGroupsSection application={application} />
      ...
      <RepresentativesSection
        representatives={
          application.representatives || application.reps || application.representative || []
        }
      />
      ...
      {(() => {
        ...
        const primaryRep = Array.isArray(reps)
          ? reps.find((r) => r.rep_order === 1 || r.is_primary === 1 || r.is_primary === true) ||
            reps[0]
          : null;
        ...
        if (!contact) {
          const fromApp = (keys) =>
            keys.reduce((v, k) => (v != null && v !== "" ? v : application?.[k]), undefined);
          const synth = {
            first_name_th: fromApp([
              "contact_first_name_th",
              "contactFirstNameTh",
              "contact_firstname_th",
              "contactFirstnameTh",
            ]),
            last_name_th: fromApp([
              "contact_last_name_th",
              "contactLastNameTh",
              "contact_lastname_th",
              "contactLastnameTh",
            ]),
            first_name_en: fromApp([
              "contact_first_name_en",
              "contactFirstNameEn",
              "contact_firstname_en",
              "contactFirstnameEn",
            ]),
            last_name_en: fromApp([
        ...
        const pick = (obj, keys) =>
          keys.reduce((v, k) => (v != null && v !== "" ? v : obj?.[k]), undefined);
        // Try to derive first/last names with many variants
        let firstNameTh = pick(contact || {}, [
          "first_name_th",
          "firstNameTh",
          "name_th",
          "nameTh",
      ...
      {((application.businessTypes &&
        (Array.isArray(application.businessTypes)
          ? application.businessTypes.length > 0
          : Object.keys(application.businessTypes).length > 0)) ||
                  ...
                  {Array.isArray(application.businessTypes)
                    ? application.businessTypes.map((businessType, index) => {
                        ...
                        if (
                          ...
                          application.businessTypeOther.length > 0
                        ) {
                          const otherDetail = application.businessTypeOther.find(
                            (other) => other.main_id === businessType.main_id,
                          );
                          return (
                            <span
                              key={index}
                              className="px-4 py-2 bg-orange-100 text-orange-800 text-sm font-semibold rounded-full border border-orange-200"
                            >
                    ...
                    : Object.entries(application.businessTypes)
                        .map(([key, value], index) => {
