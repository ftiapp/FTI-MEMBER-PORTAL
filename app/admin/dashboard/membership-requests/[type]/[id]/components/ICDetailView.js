...
const ICDetailView = ({
  ...
  const renderDocuments = () => {
    return <DocumentsSection application={application} />;
  };
  ...
  const getBusinessTypeName = (businessType) => {
    ...
    if (typeof businessType === "string") {
      if (businessType === "other") {
        const detail =
          typeof application?.businessTypeOther === "string" ? application.businessTypeOther : "";
  ...
  const userInfo = {
    ...
    lastname:
      ...
      application?.applicant_lastname ||
      application?.lastNameTh ||
      "-",
      ...
      <div className="space-y-3">
        {chapters.map((chapter, index) => {
          ...
          if (typeof chapter === "string" || typeof chapter === "number") {
            ...
            chapterId = String(
              chapter.id || chapter.province_chapter_id || chapter.MEMBER_GROUP_CODE || "",
            );
            chapterName =
              chapter.name ||
      ...
      <div className="bg-white rounded-xl shadow-sm border border-blue-200 p-8 mb-8">
        ...
        {addresses.map((addr, index) => {
          ...
          return (
            <div
              key={`address-${index}`}
              className={`${styles.bg} border ${styles.border} rounded-lg p-6 mb-6`}
            >
              <h4
                className={`text-xl font-semibold ${styles.text} mb-4 border-b ${styles.border} pb-2`}
              >
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <p className="text-sm text-gray-900">
                          {safeValue(product.name_th || product.nameTh)}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-blue-700 mb-1">
                        ...
                        <p className="text-sm text-gray-900">
                          {safeValue(product.name_en || product.nameEn)}
                        </p>
                      </div>
                    </div>
                  </div>
    ...
    return (
      <div className="bg-white rounded-xl shadow-sm border border-blue-200 p-8 mb-8 print:hidden">
        ...
        {(application?.status === 1 ||
          ...
          (application?.member_code || application?.memberCode) && (
            <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
              <h4 className="text-lg font-semibold mb-2 text-green-800">หมายเลขสมาชิก</h4>
              <p className="text-xl font-bold text-green-700">
                {safeValue(application.member_code || application.memberCode)}
              </p>
