      ...
      {application.addresses && application.addresses.length > 0 ? (
        <div className="space-y-8">
          {([...application.addresses]
            ...
            const addressType = addressTypes[key] || { label: `ที่อยู่ประเภท ${key}`, color: 'gray' };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            ...
            <p className="text-lg text-gray-900 break-all">{application.company_email || application.companyEmail || application.email || '-'}</p>
          </div>
          <div>
            <p className="text-sm font-semibold text-blue-700 mb-1">เว็บไซต์</p>
            <p className="text-lg text-gray-900 break-all">{application.company_website || application.companyWebsite || application.website || '-'}</p>
          </div>
        </div>
      )}
    </div>
