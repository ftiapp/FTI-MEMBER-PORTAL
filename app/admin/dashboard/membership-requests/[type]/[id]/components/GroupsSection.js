...
const GroupsSection = ({ industrialGroupIds, provincialChapterIds, industrialGroups, provincialChapters }) => {
  if ((!industrialGroupIds || industrialGroupIds.length === 0) && 
              ...
              <div key={index} className="flex items-center gap-2 p-2 bg-green-50 rounded-lg">
                ...
                <div className="flex-1">
                  ...
                  {provincialChapters[chapter.id || chapter] && (
                    <span className="text-xs text-gray-500 ml-2">(รหัส: {chapter.id || chapter})</span>
                  )}
                </div>
              </div>
            ))}
