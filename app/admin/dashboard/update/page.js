      ...
      >
        ...
        <div className="flex space-x-2 mb-4">
          {TABS.map(tab => (
            <button
              key={tab.key}
              onClick={() => { setActiveTab(tab.key); setSearchTerm(''); setSelectedMember(null); }}
        ...
        {/* Search Bar */}
        <SearchBar value={searchTerm} onChange={setSearchTerm} placeholder="ค้นหาด้วยชื่อบริษัทหรือรหัสสมาชิก" />
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-8 mt-5">
            <div className="text-center max-w-md mx-auto">
              ...
              <p className="mt-2 text-sm text-gray-500 mb-4">
                {searchTerm ? `ไม่พบข้อมูลสมาชิกที่ตรงกับคำค้นหา "${searchTerm}"` : 'ไม่พบข้อมูลสมาชิก'}
              </p>
              {searchTerm && (
