      ...
      {members.map((member, idx) => (
        ...
        >
          <div className="flex justify-between items-start">
            ...
            <span className={`px-2 py-1 text-xs rounded-full font-medium ${member.status === 'pending' ? 'bg-yellow-100 text-yellow-800' : member.status === 'approved' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
