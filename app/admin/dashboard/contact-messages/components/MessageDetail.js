      ...
      <div className="h-full flex flex-col">
        <div className="p-6 border-b">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-black font-semibold">
            ...
            <div>
              <span className="font-bold text-black">วันที่:</span>{" "}
              {formatDate(selectedMessage.created_at)}
            </div>
            {selectedMessage.status !== "unread" && selectedMessage.read_by_admin_name && (
              <div>
                <span className="font-bold text-black">อ่านโดย:</span>{" "}
                {selectedMessage.read_by_admin_name}
              </div>
            )}
            {selectedMessage.status === "replied" && selectedMessage.replied_by_admin_name && (
              <div>
                <span className="font-bold text-black">ตอบกลับโดย:</span>{" "}
                {selectedMessage.replied_by_admin_name}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="p-6 bg-white flex-grow overflow-y-auto">
          <div className="mb-6">
            ...
            <div className="bg-white p-4 rounded-lg border border-black">
              <p className="whitespace-pre-wrap text-black font-semibold">
