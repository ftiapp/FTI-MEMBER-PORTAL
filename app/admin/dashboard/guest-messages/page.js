  ...
  const handleReply = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Update the message status in the list
        setMessages(
          messages.map((m) => (m.id === selectedMessage.id ? { ...m, status: "replied" } : m)),
        );
  ...
  const handleCloseMessage = async () => {
    try {
      ...
      if (data.success) {
        ...
        // Update the message status in the list
        setMessages(
          messages.map((m) => (m.id === selectedMessage.id ? { ...m, status: "closed" } : m)),
        );
                  ...
                  {messages.map((message) => (
                    ...
                    >
                      <div className="flex justify-between items-start mb-1">
                        ...
                        <span
                          className={`text-xs px-2 py-1 rounded-full ${statusColors[message.status]}`}
                        >
                      ...
                      <div className="flex justify-between items-center mt-2">
                        ...
                        {message.priority && (
                          <span
                            className={`text-xs px-2 py-1 rounded-full ${priorityColors[message.priority]}`}
                          >
              ...
              <div>
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex space-x-2">
                    <span
                      className={`text-xs px-2 py-1 rounded-full ${statusColors[selectedMessage.status]}`}
                    >
                      {statusLabels[selectedMessage.status]}
                    </span>
                    {selectedMessage.priority && (
                      <span
                        className={`text-xs px-2 py-1 rounded-full ${priorityColors[selectedMessage.priority]}`}
                      >
