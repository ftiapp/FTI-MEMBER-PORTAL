  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 mb-6 print:shadow-none">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold">รายละเอียดการสมัครสมาชิก IC</h1>
          <p className="text-gray-600">วันที่สมัคร: {formatDate(application?.created_at)}</p>
        </div>

        <div className="mt-4 md:mt-0 flex items-center">
          <span
            className={`px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full border ${getStatusBadge(application?.status)}`}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <div className="bg-gray-50 p-4 rounded-md">
            <div className="mb-2">
              <span className="font-medium">คำนำหน้าชื่อ (ไทย):</span>{" "}
              {application?.prename_th || application?.prenameTh || "-"}
            </div>
            <div className="mb-2">
              <span className="font-medium">คำนำหน้าชื่อ (อังกฤษ):</span>{" "}
              {application?.prename_en || application?.prenameEn || "-"}
            </div>
            <div className="mb-2">
              <span className="font-medium">คำนำหน้าชื่อ (อื่นๆ):</span>{" "}
              {application?.prename_other || application?.prenameOther || "-"}
            </div>
            <div className="mb-2">
              <span className="font-medium">ชื่อ-นามสกุล (ไทย):</span>{" "}
              {application?.first_name_thai} {application?.last_name_thai}
            </div>
            <div className="mb-2">
              <span className="font-medium">ชื่อ-นามสกุล (อังกฤษ):</span>{" "}
        ...
        <div>
          ...
          <div className="bg-gray-50 p-4 rounded-md">
            <div className="mb-2">
              ...
              {application?.industryGroup?.name_thai || "ไม่ได้เลือก"}
            </div>
            <div>
              <span className="font-medium">สภาอุตสาหกรรมจังหวัด:</span>{" "}
              {application?.provinceChapter?.name_thai || "ไม่ได้เลือก"}
            </div>
          </div>
        </div>
      </div>
