            ...
            <p className="text-gray-800 break-words text-lg">
              {isThai ? (
                ...
                <>
                  ...
                  {(address.ADDR_ROAD_EN || address.ADDR_ROAD) && `, ${address.ADDR_ROAD_EN || address.ADDR_ROAD}`}
                  {(address.ADDR_SUB_DISTRICT_EN || address.ADDR_SUB_DISTRICT) && `, ${address.ADDR_SUB_DISTRICT_EN || address.ADDR_SUB_DISTRICT}`}
                  {(address.ADDR_DISTRICT_EN || address.ADDR_DISTRICT) && `, ${address.ADDR_DISTRICT_EN || address.ADDR_DISTRICT}`}
                  {(address.ADDR_PROVINCE_NAME_EN || address.ADDR_PROVINCE_NAME) && `, ${address.ADDR_PROVINCE_NAME_EN || address.ADDR_PROVINCE_NAME}`}
                  {(address.ADDR_POSTCODE_EN || address.ADDR_POSTCODE) && `, ${address.ADDR_POSTCODE_EN || address.ADDR_POSTCODE}`}
                </>
              )}
            </p>
          )}
