...
export default function RepresentativeTabContent({ companyInfo, representatives }) {
  ...
  const getRepresentativeData = () => {
    if (companyInfo.MEMBER_MAIN_GROUP_CODE === '000' && representatives.right.some(rep => rep.th || rep.en)) {
      return {
    ...
    } else if (companyInfo.MEMBER_MAIN_GROUP_CODE === '100' && representatives.industry.some(rep => rep.th || rep.en)) {
      return {
    ...
    } else if (companyInfo.MEMBER_MAIN_GROUP_CODE === '200' && representatives.province.some(rep => rep.th || rep.en)) {
      return {
            ...
            {representativeData.data.map((rep, index) => (
              ...
              >
                <div className={`rounded-xl overflow-hidden shadow-lg`}>
                  ...
                  <div className={`${getCardGradient(index)} h-24 flex justify-center items-center`}>
                    <motion.div 
