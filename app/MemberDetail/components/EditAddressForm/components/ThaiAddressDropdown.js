 ...
 */
export default function ThaiAddressDropdown({ formData, handleChange, itemVariants }) {
  ...
  const fetchDistricts = async (provinceName) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/thailand-address/search?query=&type=district&province=${encodeURIComponent(provinceName)}`,
      );
      const data = await response.json();
  ...
  const fetchSubDistricts = async (provinceName, districtName) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/thailand-address/search?query=&type=subdistrict&province=${encodeURIComponent(provinceName)}&district=${encodeURIComponent(districtName)}`,
      );
      const data = await response.json();
  ...
  const fetchDistrictOptions = useCallback(
    async (searchTerm) => {
      ...
      try {
        const response = await fetch(
          `/api/thailand-address/search?query=${encodeURIComponent(searchTerm)}&type=district&province=${encodeURIComponent(cleanProvinceName)}`,
        );
        const data = await response.json();
  ...
  const fetchSubDistrictOptions = useCallback(
    async (searchTerm) => {
      const cleanProvinceName = formData.ADDR_PROVINCE_NAME?.replace("จังหวัด", "").trim();
      const cleanDistrictName = formData.ADDR_DISTRICT?.replace("อำเภอ", "")
        .replace("เขต", "")
        .trim();
      ...
      try {
        const response = await fetch(
          `/api/thailand-address/search?query=${encodeURIComponent(searchTerm)}&type=subdistrict&province=${encodeURIComponent(cleanProvinceName)}&district=${encodeURIComponent(cleanDistrictName)}`,
        );
        const data = await response.json();
        ...
        if (data.success) {
          ...
          const subDistricts = data.data
            .filter(
              (item) => item.province === cleanProvinceName && item.district === cleanDistrictName,
            )
            .map((item) => ({
              text: item.text,
              province: item.province,
