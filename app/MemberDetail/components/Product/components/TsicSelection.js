...
import TsicLoadingOverlay from "./TsicLoadingOverlay";

// Import API functions
import { submitTsicUpdate, fetchTsicCodes, preloadTsicCodes } from "./api";

// Helper wrappers to emulate warning/info styles with react-hot-toast
// Usage: toastWarn("message") for warning-like toasts, toastInfo("message") for info
const toastWarn = (message, options = {}) => toast(message, { icon: "⚠️", ...options });
const toastInfo = (message, options = {}) => toast(message, { icon: "ℹ️", ...options });

const MAX_MAIN_CATEGORIES = 3;
const MAX_SUBCATEGORIES = 5;

export default function TsicSelection({
  onSuccess,
  memberCode,
  isEditMode = false,
  replacingTsicCode = null,
  editingTsicId = null,
  language = "th",
  onDataLoaded,
}) {
      ...
      if (result.success && Array.isArray(result.data) && result.data.length > 0) {
        ...
        for (const categoryCode of uniqueCategories) {
          ...
          if (category) {
            ...
            const tsicsInCategory = result.data.filter(
              (tsic) => tsic.category_code === categoryCode,
            );

            console.log(
              `Found ${tsicsInCategory.length} TSIC codes for category ${categoryCode}:`,
              tsicsInCategory,
            );
            ...
            tsicsInCategory.forEach((tsic) => {
              const subcategory = subcategories[categoryCode]?.find(
                (sub) => sub.tsic_code === tsic.tsic_code,
              );
        ...
        if (editMode.active && editMode.tsicCode) {
          ...
          if (tsicToEdit) {
            ...
            if (category) {
              ...
              const subcategoryObj = subcategories[categoryCode]?.find(
                (sub) => sub.tsic_code === tsicToEdit.tsic_code,
              ) || {
  ...
  const fetchMainCategories = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (memberCode) {
          console.log(
            "Main categories loaded successfully, now preloading TSIC codes for member code:",
            memberCode,
          );
      ...
      toast.error(
        getText(
          "ไม่สามารถโหลดหมวดหมู่หลักได้ กรุณาลองใหม่อีกครั้ง",
          "Unable to load main categories. Please try again.",
        ),
      );
    } finally {
      setIsLoading((prev) => ({ ...prev, main: false }));
  ...
  const fetchSubcategories = async (category) => {
    ...
    try {
      ...
      toast.error(
        getText(
          "ไม่สามารถโหลดหมวดหมู่ย่อยได้ กรุณาลองใหม่อีกครั้ง",
  ...
  const handleSubmit = () => {
    if (selectedMainCategories.length === 0) {
      toastWarn(
        getText("กรุณาเลือกอย่างน้อย 1 หมวดหมู่หลัก", "Please select at least 1 main category"),
      );
      return;
    }
  ...
  const confirmSubmit = async () => {
    setShowLoadingOverlay(true); // Show loading overlay and freeze page
    ...
    try {
      ...
      if (!user.id) {
        ...
        toast.error(
          getText(
            "ข้อมูลผู้ใช้ไม่ถูกต้อง กรุณาเข้าสู่ระบบใหม่อีกครั้ง",
      ...
      if (!allSelectedSubcategories || allSelectedSubcategories.length === 0) {
        ...
        toast.error(
          getText(
            "กรุณาเลือกรหัส TSIC อย่างน้อย 1 รายการ",
            "Please select at least one TSIC code.",
          ),
        );
      ...
      let tsicCodes = allSelectedSubcategories
        .map((sub) => {
      ...
      if (!result.success) {
        ...
        // กรณีข้อผิดพลาดอื่นๆ
        throw new Error(
          result.message || getText("ไม่สามารถส่งข้อมูลได้", "Unable to submit data"),
        );
      }
      ...
      toast.success(getText("บันทึกรหัส TSIC เรียบร้อยแล้ว", "TSIC codes saved successfully."));
    } catch (error) {
  ...
  return (
    <>
      {/* Loading Overlay - Freeze page during submission */}
      <TsicLoadingOverlay
        isVisible={showLoadingOverlay}
        message={getText("กำลังส่งข้อมูล TSIC", "Submitting TSIC data")}
      />
