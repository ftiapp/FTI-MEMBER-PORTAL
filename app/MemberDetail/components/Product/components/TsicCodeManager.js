...
export default function TsicCodeManager({ memberCode, language = 'th', memberType = '', memberGroupCode = '', typeCode = '' }) {
  ...
  const handleAdd = () => {
    if (checkCooldown()) {
      alert(getText('ท่านได้ส่งคำขอแก้ไขข้อมูลแล้ว หากต้องการเพิ่ม/แก้ไข ข้อมูล กรุณารอ 5 วินาที ขออภัยในความไม่สะดวก', 'You have already submitted a request. If you want to add/edit data, please wait 5 seconds. We apologize for the inconvenience.'));
      return;
    }
    setIsLoading(true);
    setMode('add');
          ...
          {isLoading && (
            <div className="bg-white p-6 rounded-lg shadow mb-4">
              <div className="flex items-center justify-center space-x-3">
                ...
                <p className="text-gray-700">{getText('ระบบกำลังค้นหารหัส TSIC ที่มีอยู่แล้ว...กรุณารอสักครู่', 'Searching for existing TSIC codes... Please wait')}</p>
              </div>
            </div>
          )}
