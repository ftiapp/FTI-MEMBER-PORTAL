 ...
 */
export async function preloadTsicCodes(
  memberCode,
  mainCategories,
    ...
    if (result.success && Array.isArray(result.data) && result.data.length > 0) {
      ...
      for (const categoryCode of uniqueCategories) {
        ...
        if (category) {
          ...
          const tsicsInCategory = result.data.filter((tsic) => tsic.category_code === categoryCode);
...
export async function fetchMainCategories(query = "") {
  try {
    ...
    const response = await fetch(
      `/api/tsic/categories${query ? `?search=${encodeURIComponent(query)}` : ""}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
 ...
 * @returns {Object} - Result with success status and message
 */
export async function submitTsicUpdate(
  email,
  memberCode,
  tsicCodes,
  ...
  try {
    ...
    const response = await fetch("/api/member/tsic-codes/direct-update", {
      ...
      body: JSON.stringify({
        ...
        memberCode,
        tsicCodes:
          tsicCodesWithCategories.length > 0
            ? tsicCodesWithCategories
            : validTsicCodes.map((code) => ({ category_code: "00", tsic_code: code })),
