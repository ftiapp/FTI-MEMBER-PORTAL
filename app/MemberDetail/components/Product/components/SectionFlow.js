...
export default function SectionFlow({ currentStep, language = 'th' }) {
  const steps = [
    ...
    { 
      ...
      icon: (
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5h6a2 2 0 012 2v4h-2V7H9v4H7V7a2 2 0 012-2z" />
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13h14a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2z" />
        </svg>
      )
    },
    { 
  ...
  return (
    <div className="mb-8">
      ...
      <div className="relative px-8"> {/* เพิ่ม padding ซ้าย-ขวา */}
        {/* เส้นเชื่อมต่อแนวนอนสีเทา - ปรับให้ไม่ชนขอบ */}
        <div className="absolute top-7 left-1/2 transform -translate-x-1/2 h-2 bg-gray-200 z-0" 
             style={{ width: 'calc(100% - 120px)' }}> {/* ปรับขนาดและตำแหน่ง */}
        </div>
          ...
          {steps.map((step) => {
            ...
            return (
              <div key={step.id} className="flex flex-col items-center flex-1">
                ...
                <span className={`mt-3 text-sm text-center font-medium max-w-24 leading-tight ${
                  isActive 
                    ? 'text-blue-600' 
                    : isCompleted 
                      ? 'text-blue-600' 
