      ...
      >
        <div className="space-y-8">
          {selectedMainCategories.map((category) => {
            ...
            const totalPages = Math.ceil((filteredSubcategories[category.category_code]?.length || 0) / itemsPerPage);
            const startIndex = (currentPageForCategory - 1) * itemsPerPage;
            const paginatedSubcategories = filteredSubcategories[category.category_code]?.slice(
              startIndex,
              ...
              <div key={category.category_code} className="border-b pb-6 last:border-b-0 last:pb-0">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-3">
                  ...
                  <div className="relative w-full sm:w-64">
                    <input
                      type="text"
                      placeholder={language === 'th' ? 'ค้นหาหมวดหมู่ย่อย...' : 'Search subcategories...'}
                          ...
                          >
                            ...
                            <div className="min-w-0 flex-grow">
                              ...
                              <p className="text-sm text-gray-600 break-words">
                                {language === 'th' ? subcategory.description : (subcategory.description_EN || subcategory.description)}
                              </p>
                            </div>
                            <button 
                              onClick={(e) => {
                        ...
                        {paginatedSubcategories.map((subcategory) => {
                          ...
                          return (
                            <SubcategoryItem
                              ...
                              onSelect={() => handleSubcategoryChange(category.category_code, subcategory, !isSelected)}
                              language={language}
                            />
                          );
                        })}
