...
export async function POST(request, { params }) {
  ...
  try {
    ...
    // Build files map similar to submit route
    const files = {};
    ...
    // Verify ownership (no explicit transaction needed here)
    const mainRows = await executeQueryWithoutTransaction(
      "SELECT user_id FROM MemberRegist_OC_Main WHERE id = ? LIMIT 1",
      [id],
    );
    ...
    // Determine which document types we are replacing
    ...
    // 1) Fetch existing documents for these types and delete from Cloudinary
    for (const docType of docTypesToReplace) {
      const existingDocs = await executeQueryWithoutTransaction(
        "SELECT id, cloudinary_id FROM MemberRegist_OC_Documents WHERE main_id = ? AND document_type = ?",
        [id, docType],
      );
      ...
      if (existingDocs && existingDocs.length > 0) {
        ...
        await executeQueryWithoutTransaction(
          "DELETE FROM MemberRegist_OC_Documents WHERE main_id = ? AND document_type = ?",
          [id, docType],
        );
      }
    }

    // 2) Upload new files and insert document rows
    const uploadedDocuments = {};

    // Upload files to Cloudinary (similar to submit route)
    for (const fieldName of Object.keys(files)) {
      ...
      if (fieldName === "productionImages" && Array.isArray(fileValue)) {
        for (let index = 0; index < fileValue.length; index++) {
          ...
          try {
            ...
            console.error("Error uploading production image in update-documents:", uploadError);
          }
        }
      }
      // authorizedSignatures[N]: single file per signatory
        ...
        if (indexMatch) {
          ...
          try {
            ...
            console.error("Error uploading authorized signature in update-documents:", uploadError);
          }
        }
      }
      // Single-file fields
        ...
        try {
          ...
          console.error(`Error uploading file for ${fieldName} in update-documents:`, uploadError);
        }
      }
    }

    // Link authorizedSignatures to signature_name_id like submit route
    ...
    try {
      ...
      // Simple mapping by order
      signatureRecords.forEach((record, index) => {
        signatureNameIds[index] = record.id;
      });
    } catch (error) {
      console.error("Error fetching signature name IDs in update-documents:", error);
    }

    // Insert uploaded documents into MemberRegist_OC_Documents
    for (const documentKey in uploadedDocuments) {
      ...
      try {
        ...
        console.error(`Error inserting document ${documentKey} in update-documents:`, dbError);
      }
    }

    return NextResponse.json({ success: true, message: "อัปเดตเอกสารแนบเรียบร้อยแล้ว" });
  } catch (error) {
    console.error("[OC API] Error in update-documents:", error);
    return NextResponse.json(
