...
export async function POST(request, { params }) {
  ...
  try {
    ...
    const mainRows = await executeQueryWithoutTransaction(
      "SELECT user_id FROM MemberRegist_IC_Main WHERE id = ? LIMIT 1",
      [id],
    );
    ...
    for (const fieldName of Object.keys(files)) {
      ...
      try {
        ...
        const result = await uploadToCloudinary(Buffer.from(buffer), file.name, {
          folder: "FTI_PORTAL_IC_member_DOC",
          resource_type: "auto",
        });
      ...
      try {
        await executeQueryWithoutTransaction(
          `INSERT INTO MemberRegist_IC_Documents
             (main_id, document_type, file_name, file_path, file_size, mime_type, cloudinary_url, created_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, NOW())`,
          [
            id,
            doc.document_type,
        ...
        console.error(`Error inserting document ${documentKey} in update-documents (IC):`, dbError);
      }
    }

    return NextResponse.json({ success: true, message: "อัปเดตเอกสารแนบเรียบร้อยแล้ว" });
  } catch (error) {
    console.error("[IC API] Error in update-documents:", error);
    return NextResponse.json(
