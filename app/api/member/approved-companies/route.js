  ...
  try {
    ...
    try {
      ...
      if (approvedCompanies.length > 0) {
        ...
        const validCompanies = approvedCompanies.filter(company => company.COMP_PERSON_CODE && company.REGIST_CODE);
        ...
        if (validCompanies.length > 0) {
          ...
          try {
            ...
            validCompanies.forEach(company => {
              conditions.push(`(COMP_PERSON_CODE = @param${paramIndex} AND REGIST_CODE = @param${paramIndex + 1})`);
    ...
    const companiesWithStatus = approvedCompanies.map(company => {
      ...
      const key = company.COMP_PERSON_CODE && company.REGIST_CODE ? 
        `${company.COMP_PERSON_CODE}_${company.REGIST_CODE}` : null;
      
      return {
        ...company,
        memberStatus: (key && memberStatusMap[key]) ? memberStatusMap[key] : {
          statusCode: '',
