...
export async function POST(request) {
  try {
    ...
    console.log('Request data received:', { ...requestData, tsicCodes: requestData.tsicCodes ? `${requestData.tsicCodes.length} items` : 'none' });
    
    const { action, memberCode, categoryCode, tsicCode, status, userId: bodyUserId, tsicCodes } = requestData;
    ...
    switch (action) {
      case 'add_multiple':
        ...
        try {
          ...
          try {
            ...
            const validNewCodes = newCodes.filter(code => 
              code && code.category_code && code.tsic_code
            );
            ...
            const insertValues = validNewCodes.map(code => 
      ...
      case 'delete':
        ...
        // Delete TSIC code
        await query(
          'DELETE FROM member_tsic_codes WHERE member_code = ? AND tsic_code = ?',
          [memberCode, tsicCode]
        );
