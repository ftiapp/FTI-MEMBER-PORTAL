...
export async function POST(request) {
  try {
    ...
    if (existingLogs.length > 0) {
      ...
      console.log(`Certificate action count updated: User ${userId} ${actionType}ed ${language} certificate for member ${memberCode} (count: ${newCount})`);
    } else {
      ...
      console.log(`New certificate action logged: User ${userId} ${actionType}ed ${language} certificate for member ${memberCode} (count: 1)`);
    }
