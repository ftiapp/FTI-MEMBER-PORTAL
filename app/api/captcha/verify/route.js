...
export async function POST(req) {
  try {
    ...
    if (!secret) {
      return NextResponse.json({ ok: false, error: "Server is not configured with RECAPTCHA_SECRET_KEY" }, { status: 500 });
    }
    ...
    if (!data.success) {
      return NextResponse.json({ ok: false, reason: "verify_failed", result: data }, { status: 400 });
    }
    ...
    if (isV3) {
      ...
      const configuredExpectedAction = (process.env.RECAPTCHA_EXPECTED_ACTION || "register").toString();
      const allowedHostnamesEnv = (process.env.RECAPTCHA_ALLOWED_HOSTNAMES || "ftimemberportal-529sy.kinsta.app,localhost").toString();
      const allowedHostnames = allowedHostnamesEnv.split(",").map(s => s.trim().toLowerCase()).filter(Boolean);

      // Validate score
      if (data.score < minScore) {
        return NextResponse.json({ ok: false, reason: "low_score", minScore, score: data.score, result: data }, { status: 400 });
      }
      ...
      if (data.action && actionToCheck && data.action !== actionToCheck) {
        return NextResponse.json({ ok: false, reason: "action_mismatch", expected: actionToCheck, actual: data.action, result: data }, { status: 400 });
      }
      ...
      if (data.hostname) {
        ...
        if (!hostOk) {
          return NextResponse.json({ ok: false, reason: "hostname_not_allowed", hostname: data.hostname, allowedHostnames }, { status: 400 });
        }
      }
    } else {
