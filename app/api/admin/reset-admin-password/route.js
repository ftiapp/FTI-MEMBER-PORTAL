...
import { sendAdminInviteEmail } from "../../../lib/postmark";
...
export async function POST(request) {
  try {
    ...
    if (Number(target.admin_level) >= 5) {
      return NextResponse.json(
        { success: false, message: "ไม่สามารถรีเซ็ทรหัสผ่านของ SuperAdmin ได้" },
        { status: 400 },
      );
    }
    ...
    await query(
      ...
      [
        ...
        admin.id,
        target.admin_level,
        target.can_create ? 1 : 0,
        target.can_update ? 1 : 0,
        expiresAt,
      ],
    );
    ...
    await logAdminAction(
      ...
      adminId,
      { email, adminLevel: target.admin_level },
      request,
    );

    return NextResponse.json({
      success: true,
      message: "รีเซ็ทรหัสผ่านและส่งอีเมลเชิญใหม่เรียบร้อยแล้ว",
    });
  } catch (error) {
    console.error("Error resetting admin password:", error);
    return NextResponse.json(
      { success: false, message: "เกิดข้อผิดพลาดในการรีเซ็ทรหัสผ่าน" },
      { status: 500 },
    );
  }
}
