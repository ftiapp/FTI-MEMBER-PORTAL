  ...
  try {
    ...
    try {
      ...
      switch (type) {
        ...
        case "ic":
          companyQuery =
            'SELECT id_card_number as tax_id, CONCAT(first_name_th, " ", last_name_th) as company_name_th FROM ICmember_Info WHERE id = ?';
          break;
      }
    ...
    const [mainRows] = await connection.execute(`SELECT user_id FROM ${tableName} WHERE id = ?`, [
      id,
    ]);
    ...
    return NextResponse.json(
      { success: false, message: "เกิดข้อผิดพลาดในการบันทึกหมายเหตุ" },
      { status: 500 },
    );
  }
}
