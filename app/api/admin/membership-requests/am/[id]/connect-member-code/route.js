...
export async function POST(request, { params }) {
  try {
    ...
    try {
      // Get the tax_id from MemberRegist_AM_Main
      const [memberRows] = await mysqlConnection.execute(
        'SELECT tax_id, company_name FROM MemberRegist_AM_Main WHERE id = ? AND status = 1',
        [id]
      );
      ...
      const [existingCodeRows] = await mysqlConnection.execute(
        'SELECT member_code FROM MemberRegist_AM_Main WHERE id = ? AND member_code IS NOT NULL',
        [id]
      );
      ...
      try {
        ...
        const result = await mssqlPool.request()
          .input('tax_id', sql.VarChar, tax_id)
          .query(`
            SELECT  [MEMBER_CODE]
                  ,[REGIST_CODE]
                  ,[COMP_PERSON_CODE]
        ...
        // Update the MemberRegist_AM_Main table with the MEMBER_CODE
        await mysqlConnection.execute(
          'UPDATE MemberRegist_AM_Main SET member_code = ?, connection_status = 1, updated_at = NOW() WHERE id = ?',
          [member_code, id]
        );
        ...
        await mysqlConnection.execute(
          ...
          [
            ...
            id,
            `เชื่อมต่อหมายเลขสมาชิก ${member_code} สำหรับสมาชิกประเภท สส (สามัญ-สมาคมการค้า) ID: ${id} เลขประจำตัวผู้เสียภาษี: ${tax_id} ชื่อสมาคม: ${company_name}`,
