  ...
  try {
    ...
    try {
      ...
      const [existingCodeRows] = await mysqlConnection.execute(
        'SELECT member_code FROM MemberRegist_AC_Main WHERE id = ? AND member_code IS NOT NULL',
        [id]
      );
      ...
      try {
        ...
        // Update the MemberRegist_AC_Main table with the MEMBER_CODE
        await mysqlConnection.execute(
          'UPDATE MemberRegist_AC_Main SET member_code = ?, connection_status = 1, updated_at = NOW() WHERE id = ?',
          [member_code, id]
        );
        ...
        await mysqlConnection.execute(
          ...
          [
            ...
            id,
            `เชื่อมต่อหมายเลขสมาชิก ${member_code} สำหรับสมาชิกประเภท สน (สมทบ-นิติบุคคล) ID: ${id} เลขประจำตัวผู้เสียภาษี: ${tax_id} ชื่อบริษัท: ${company_name}`,
