  ...
  try {
    ...
    try {
      const [memberRows] = await mysqlConnection.execute(
        'SELECT id_card_number, user_id, CONCAT(first_name_th, " ", last_name_th) as full_name FROM ICmember_Info WHERE id = ? AND status = 1',
        [id],
      );
      ...
      const { id_card_number, full_name, user_id } = memberRows[0];
      ...
      try {
        ...
        const result = await mssqlPool.request().input("tax_id", sql.VarChar, id_card_number).query(`
