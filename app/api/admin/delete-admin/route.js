...
export async function POST(request) {
  try {
    ...
    const rows = await query('SELECT id, username, admin_level FROM admin_users WHERE id = ?', [adminId]);
    if (rows.length === 0) {
      return NextResponse.json({ success: false, message: 'ไม่พบผู้ดูแลระบบ' }, { status: 404 });
    }
    const target = rows[0];
    if (Number(target.admin_level) >= 5) {
      return NextResponse.json({ success: false, message: 'ไม่สามารถลบ SuperAdmin ได้' }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: false, message: 'เกิดข้อผิดพลาดในการลบผู้ดูแลระบบ' }, { status: 500 });
  }
}
