...
export async function POST(request) {
  try {
    ...
    const targetAdmin = await query(`SELECT admin_level FROM admin_users WHERE id = ?`, [adminId]);
    ...
    // Update admin status
    await query(`UPDATE admin_users SET is_active = ? WHERE id = ?`, [isActive ? 1 : 0, adminId]);
