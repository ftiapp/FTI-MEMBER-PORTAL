  ...
  try {
    ...
    if (!trimmedName) {
      ...
      return NextResponse.json(
        { success: false, message: "กรุณาระบุชื่อผู้ดูแลระบบ" },
        { status: 400 },
      );
    }
    if (trimmedName.length > 100) {
      await rollbackTransaction(connection);
      return NextResponse.json(
        { success: false, message: "ชื่อยาวเกินไป (สูงสุด 100 ตัวอักษร)" },
        { status: 400 },
      );
    }
    ...
    const insertRes = await executeQuery(
      ...
      [
        ...
        hashed,
        invite.admin_level,
        invite.can_create ? 1 : 0,
        invite.can_update ? 1 : 0,
    ...
    if (invite.inviter_id) {
      try {
        await logAdminAction(
          ...
          "accept_invite",
          newAdminId,
          { email, adminLevel: invite.admin_level },
          request,
        );
      } catch (e) {
