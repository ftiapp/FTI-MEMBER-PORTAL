...
export async function GET(request) {
  try {
    ...
    const cacheKey = JSON.stringify({ year: year ?? "all", rangeStart, rangeEnd, memberType });
    ...
    if (memberType === "OC") {
      queryText = `SELECT a.province AS name, COUNT(*) AS count
         ...
         WHERE a.address_type = '2' AND ${yearCondition}MONTH(m.created_at) BETWEEN ? AND ?
         GROUP BY a.province
         ORDER BY count DESC, a.province ASC`;
      params = [...(year ? [year] : []), rangeStart, rangeEnd];
    ...
    if (memberType === "OC") {
      ...
      monthlyQuery = `
        ...
        GROUP BY month
      `;
      monthlyParams = year ? [year, year, year, year] : [];
    }
