    ...
    try {
      ...
      try {
        ...
        if (!spCheck || !spCheck[0] || !spCheck[0].sp_id) {
          console.error(
            "Stored procedure sp_GetMemberDetailByMemberCode_FTI_PORTAL does not exist",
          );
      ...
      if (!result || result.length === 0) {
        ...
        return NextResponse.json({ error: "ไม่พบข้อมูลสมาชิก", memberCode }, { status: 404 });
      }
    } catch (spError) {
