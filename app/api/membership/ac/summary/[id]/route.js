...
export async function GET(request, { params }) {
  try {
    ...
    const hasOtherBusinessType =
      Array.isArray(businessTypesRows) &&
      businessTypesRows.find((bt) => bt.business_type === "other");
    ...
    const productsQuery = await query("SELECT * FROM MemberRegist_AC_Products WHERE main_id = ?", [
      id,
    ]);
    ...
    const businessTypes = Array.isArray(businessTypesRows)
      ? businessTypesRows.map((bt) => ({
          id: bt.business_type,
          name: BUSINESS_TYPE_MAP[bt.business_type] || bt.business_type,
          details:
            bt.business_type === "other" && businessTypeOther ? businessTypeOther.detail : null,
        }))
      : [];
    ...
    const transformedData = {
      ...
      businessTypes: Array.isArray(businessTypesRows)
        ? businessTypesRows.reduce((acc, bt) => {
      ...
      companyRegistration: documentsRows.find((doc) => doc.document_type === "companyRegistration")
        ? {
            name:
              documentsRows.find((doc) => doc.document_type === "companyRegistration")?.file_name ||
              "ไฟล์ถูกอัปโหลดแล้ว",
            fileUrl:
              documentsRows.find((doc) => doc.document_type === "companyRegistration")
                ?.cloudinary_url ||
              documentsRows.find((doc) => doc.document_type === "companyRegistration")?.file_path,
            fileType: documentsRows.find((doc) => doc.document_type === "companyRegistration")
              ?.mime_type,
            fileSize: documentsRows.find((doc) => doc.document_type === "companyRegistration")
              ?.file_size,
            cloudinaryId: documentsRows.find((doc) => doc.document_type === "companyRegistration")
              ?.cloudinary_id,
          }
        : null,
      ...
      companyStamp: documentsRows.find((doc) => doc.document_type === "companyStamp")
        ? {
            name:
              documentsRows.find((doc) => doc.document_type === "companyStamp")?.file_name ||
              "ไฟล์ถูกอัปโหลดแล้ว",
            fileUrl:
              documentsRows.find((doc) => doc.document_type === "companyStamp")?.cloudinary_url ||
              documentsRows.find((doc) => doc.document_type === "companyStamp")?.file_path,
            ...
            cloudinaryId: documentsRows.find((doc) => doc.document_type === "companyStamp")
              ?.cloudinary_id,
          }
        : null,
