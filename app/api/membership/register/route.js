...
export async function POST(request) {
  try {
    ...
    if (checkResult.rows.length > 0) {
      return NextResponse.json({ message: "เลขบัตรประชาชนนี้มีในระบบแล้ว" }, { status: 400 });
    }
    ...
    if (memberTypeResult.rows.length === 0) {
      return NextResponse.json({ message: "ไม่พบประเภทสมาชิก" }, { status: 400 });
    }
