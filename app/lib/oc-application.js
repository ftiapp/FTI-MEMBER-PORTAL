 ...
 */
export async function updateOCApplication(membershipId, formData, userId, rejectionId, userComment) {
  ...
  try {
    ...
    await addComment(connection, 'oc', membershipId, userId, null, 'user_resubmit', 
      'ผู้ใช้ได้แก้ไขข้อมูลและส่งใบสมัครใหม่', null, formData);

    // บันทึก comment จากผู้ใช้ (ถ้ามี)
    if (userComment && userComment.trim() !== '') {
      await addComment(connection, 'oc', membershipId, userId, null, 'user_message', userComment, null, null);
    }
...
async function updateMainInfo(connection, membershipId, formData) {
  const { 
    companyName, companyNameEng, taxId, companyEmail, companyPhone, 
    industrialGroup, provincialChapter, products, otherBusinessType 
  } = formData;
  ...
  `, [
    companyName, companyNameEng, taxId, companyEmail, companyPhone,
    industrialGroup || null, provincialChapter || null, products, otherBusinessType,
    membershipId
  ]);
}
...
async function updateContactPerson(connection, membershipId, formData) {
  const { 
    contactPersonFirstName, contactPersonLastName, contactPersonPosition, 
...
async function updateRepresentatives(connection, membershipId, representatives) {
  ...
  await connection.execute('DELETE FROM MemberRegist_OC_Representative WHERE membership_id = ?', [membershipId]);
  ...
  if (representatives && representatives.length > 0) {
    ...
    const values = representatives.map(rep => [
      membershipId, rep.idCardNumber, rep.firstNameThai, rep.lastNameThai,
      rep.firstNameEnglish, rep.lastNameEnglish, rep.position, rep.email, rep.phone, rep.isPrimary || false
    ]);
...
async function updateBusinessTypes(connection, membershipId, businessTypes) {
  ...
  await connection.execute('DELETE FROM MemberRegist_OC_Business_Type WHERE membership_id = ?', [membershipId]);

  // Insert new business types
  if (businessTypes && businessTypes.length > 0) {
    const query = 'INSERT INTO MemberRegist_OC_Business_Type (membership_id, business_type_id) VALUES ?';
 ...
 * เพิ่ม comment ในตาราง MemberRegist_Comments
 */
async function addComment(connection, membershipType, membershipId, userId, adminId, commentType, commentText, rejectionReason = null, dataChanges = null) {
  await connection.execute(`
