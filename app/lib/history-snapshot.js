...
export async function createOCSnapshot(connection, membershipId, reason = 'rejection', snapshotBy) {
  ...
  const [mainResult] = await connection.execute(
    ...
    [
      main.id, main.user_id, main.member_code, main.company_name_th, main.company_name_en, main.tax_id,
      main.company_email, main.company_phone, main.company_phone_extension, main.status, main.factory_type,
      main.number_of_employees, main.registered_capital, main.production_capacity_value, main.production_capacity_unit,
      main.sales_domestic, main.sales_export, main.shareholder_thai_percent, main.shareholder_foreign_percent,
      main.revenue_last_year, main.revenue_previous_year, main.rejection_reason, main.admin_note,
      main.admin_note_by, main.admin_note_at, reason, snapshotBy
    ]
  );
  ...
  for (const rep of representatives) {
    await connection.execute(
      ...
      [
        historyId, membershipId, rep.prename_th, rep.prename_en, rep.prename_other, rep.prename_other_en,
        rep.first_name_th, rep.last_name_th, rep.first_name_en, rep.last_name_en,
        rep.position, rep.email, rep.phone
      ]
    );
  }
  ...
  for (const addr of addresses) {
    await connection.execute(
      ...
      [
        historyId, membershipId, addr.building || null, addr.address_type || null, addr.address_number || null,
        addr.moo || null, addr.soi || null, addr.street || null, addr.sub_district || null, addr.district || null,
        addr.province || null, addr.postal_code || null, addr.phone || null, addr.phone_extension || null,
        addr.email || null, addr.website || null
      ]
    );
  }
  ...
  for (const cp of contactPersons) {
    await connection.execute(
      ...
      [
        historyId, membershipId,
        cp.prename_th || null, cp.prename_en || null, cp.prename_other || null, cp.prename_other_en || null,
        cp.first_name_th || null, cp.last_name_th || null, cp.first_name_en || null, cp.last_name_en || null,
        cp.position || null, cp.email || null, cp.phone || null,
        cp.type_contact_id || null, cp.type_contact_name || null, cp.type_contact_other_detail || null,
  ...
  for (const doc of documents) {
    await connection.execute(
      ...
      [
        historyId, membershipId,
        doc.document_type || null, doc.file_name || null, doc.file_path || null,
        doc.file_size || null, doc.mime_type || null, doc.cloudinary_id || null, doc.cloudinary_url || null
      ]
    );
  }
  ...
  for (const sig of signatures) {
    await connection.execute(
      ...
      [
        historyId, membershipId,
        sig.prename_th || null, sig.prename_en || null, sig.prename_other || null, sig.prename_other_en || null,
        sig.first_name_th || null, sig.last_name_th || null, sig.first_name_en || null, sig.last_name_en || null,
        sig.position_th || null, sig.position_en || null
      ]
    );
  }
...
export async function createACSnapshot(connection, membershipId, reason = 'rejection', snapshotBy) {
  ...
  const [mainResult] = await connection.execute(
    ...
    [
      main.id, main.user_id, main.member_code, main.company_name_th, main.company_name_en, main.tax_id,
      main.company_email, main.company_phone, main.company_phone_extension, main.status, main.number_of_employees,
      main.registered_capital, main.sales_domestic, main.sales_export, main.shareholder_thai_percent,
      main.shareholder_foreign_percent, main.revenue_last_year, main.revenue_previous_year,
      main.rejection_reason, main.admin_note, reason, snapshotBy
    ]
  );
...
export async function createAMSnapshot(connection, membershipId, reason = 'rejection', snapshotBy) {
  ...
  const [mainResult] = await connection.execute(
    ...
    [
      main.id, main.user_id, main.member_code, main.company_name_th, main.company_name_en, main.tax_id,
      main.company_email, main.company_phone, main.company_phone_extension, main.status, main.number_of_employees,
      main.registered_capital, main.rejection_reason, main.admin_note, reason, snapshotBy
    ]
  );
  ...
  for (const rep of representatives) {
    await connection.execute(
      ...
      [
        historyId, membershipId,
        rep.prename_th || null, rep.prename_en || null, rep.prename_other || null, rep.prename_other_en || null,
        rep.first_name_th || null, rep.last_name_th || null, rep.first_name_en || null, rep.last_name_en || null,
        rep.position || null, rep.email || null, rep.phone || null
      ]
    );
  }
...
export async function createICSnapshot(connection, membershipId, reason = 'rejection', snapshotBy) {
  ...
  const [mainResult] = await connection.execute(
    ...
    [
      main.id, main.user_id, main.member_code, main.prename_th, main.prename_en, main.prename_other,
      main.first_name_th, main.last_name_th, main.first_name_en, main.last_name_en, main.id_card_number,
      main.email, main.phone, main.status, main.rejection_reason, main.admin_note, reason, snapshotBy
    ]
  );
  ...
  for (const rep of representatives) {
    await connection.execute(
      ...
      [
        historyId, membershipId, rep.prename_th || null, rep.prename_en || null, rep.prename_other || null, rep.prename_other_en || null,
        rep.first_name_th || null, rep.last_name_th || null, rep.first_name_en || null, rep.last_name_en || null,
        rep.position || null, rep.email || null, rep.phone || null
      ]
    );
 ...
 */
export async function createSnapshot(connection, membershipType, membershipId, reason = 'rejection', snapshotBy) {
