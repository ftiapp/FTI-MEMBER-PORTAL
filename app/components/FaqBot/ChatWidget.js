...
const formatMessageWithLinks = (message) => {
  ...
  const boldTerms = ["ที่อยู่", "สินค้า/บริการ", "โลโก้", "Social Media", "TSIC"];
...
const ChatWidget = () => {
          ...
          setMessages([
            {
              type: "bot",
              content:
                "สวัสดีครับ! ผมเป็น FTI MEMBER PORTAL CHAT BOT สามารถถามคำถามเกี่ยวกับการใช้งานระบบได้เลยครับ",
    ...
    if (greetings.some((greeting) => messageText.toLowerCase().includes(greeting.toLowerCase()))) {
      setMessages((prev) => [
        ...prev,
        {
          ...
          suggestions: ["วิธีการสมัครสมาชิก", "วิธีการยืนยันตัวตน", "วิธีการแก้ไขข้อมูลที่อยู่"],
        },
      ]);
      return;
    }
      ...
      if (data.success && data.data) {
        ...
        setMessages((prev) => [
          ...prev,
          {
            type: "bot",
            content:
              'ขออภัยครับ ผมไม่พบคำตอบสำหรับคำถามนี้ คุณสามารถติดต่อเจ้าหน้าที่โดยตรงได้ที่แท็บ "ติดต่อเรา" ในหน้าแดชบอร์ด',
  ...
  return (
    <div className={styles.chatWidgetContainer}>
      {/* Chat button */}
      <button className={styles.chatButton} onClick={toggleChat} aria-label="เปิด/ปิดแชท">
        {isOpen ? (
          ...
          <>
            <span className={styles.chatIcon} style={{ fontSize: "24px" }}>
              💬
            </span>
      ...
      {isOpen && (
        <div className={styles.chatWindow}>
          <div className={styles.chatHeader}>
            ...
            <button className={styles.minimizeButton} onClick={toggleChat} aria-label="ปิดแชท">
              ✕
            </button>
          </div>
