...
export default function OptimizedFileUploader({
  ...
  const handleFileSelect = (e) => {
    ...
    if (oversizedFiles.length > 0) {
      setError(
        `ไฟล์ต่อไปนี้มีขนาดเกิน ${maxFileSize} MB: ${oversizedFiles.map((f) => f.name).join(", ")}`,
      );
      return;
    }
  ...
  return (
    <div className="w-full max-w-md mx-auto p-4 bg-white rounded-lg shadow">
      ...
      {/* แสดงข้อผิดพลาด */}
      {error && <div className="mb-4 p-2 bg-red-50 text-red-600 text-sm rounded-md">{error}</div>}
      ...
      {uploading && (
        <div className="mb-4">
          <div className="flex justify-between text-xs text-gray-600 mb-1">
            <span>
              กำลังอัปโหลด... ({progress.completed}/{progress.total})
            </span>
          ...
          {progress.currentFile && (
            <div className="text-xs text-gray-500 mt-1">ไฟล์ปัจจุบัน: {progress.currentFile}</div>
          )}
        </div>
      )}
